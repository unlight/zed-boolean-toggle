...
impl BooleanToggleExtension {
    fn get_or_download_lsp(&mut self) -> zed::Result<String> {
        ...
        let asset_name = match (os, arch) {
            (zed::Os::Mac, zed::Architecture::Aarch64) => "boolean-toggle-lsp-aarch64-apple-darwin",
            (zed::Os::Mac, zed::Architecture::X8664) => "boolean-toggle-lsp-x86_64-apple-darwin",
            // Not compiling
        ...
        let lsp_path = format!("bin/{}", asset_name);
